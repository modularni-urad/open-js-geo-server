...
export function detail (layerid, knex, schema = null) {
  return getQB(knex, TABLE_NAMES.LAYERS, schema).where({ id: layerid }).first()
    .then(info => {
...
export function list (query, knex, schema = null) {
  ...
  return getQB(knex, TABLE_NAMES.LAYERS, schema).paginate({ perPage, currentPage })
}

export function create (data, uid, knex, schema = null) {
  Object.assign(data, { owner: uid })
  return getQB(knex, TABLE_NAMES.LAYERS, schema).returning('id').insert(data)
}

export async function modify (layerid, data, knex, schema = null) {
  const change = _.omit(data, ['id', 'created', 'owner'])
  return getQB(knex, TABLE_NAMES.LAYERS, schema).where({ id: layerid }).update(change)
}
